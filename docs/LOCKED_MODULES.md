...
最近锁定任务：`TASK-V8.10.10-RELEASE-LOCK-PUBLISH-040`
...
### TASK-MARKET-MONITOR-019：行情监控中心锁定

锁定基线：

- 正式版本：v8.7.0
- 功能 commit：`5d127e36cfc3f5021e5c3bbca4c598ca62a4c09d`
- 自动化测试：1361/1361
- Release build：0 warning / 0 error
- 人工功能验收：通过
- 深色主题验收：通过
...
左侧原“行情监控”固定作为独立可点击入口。

不得新增第二个行情监控入口、改名、改变导航顺序、改变导航图标或映射到 `ManualEntryScope`。

#### B. 窗口行为

行情监控中心固定为独立、单实例窗口；Owner 为 MainWindow；使用 `Show`，不使用 `ShowDialog`；重复点击只 `Activate`；关闭后释放引用；`ShowInTaskbar=False`。

#### C. 监控标的

固定展示以下并集：

- `MarketSymbolNormalizer.DefaultTopBarItems()` 固定 6 项；
- 启用策略 ETF；
- 启用策略跟踪指数；
- 当前场内 ETF 持仓；
- 启用 OTC 通道；
...
去重键固定为 `MarketType + 标准化 Symbol`。

#### D. 数据边界

行情监控中心只读取行情缓存、数据源状态、策略配置、持仓和 OTC 通道。

不得直接联网、调用 `MarketDataClient`、调用 `MarketDataRefreshService`、写数据库、写 TradeLog、写 `runtime_log`、触发回放、触发策略、触发委托或触发预警。
...
锁定当前来源阈值及 `ReceivedAt` 口径，不得增加交易所开闭市推断：

- 腾讯 ETF：30 秒内正常，30 至 120 秒延迟，超过 120 秒过期；
- 东方财富实时：60 秒内正常，60 至 180 秒延迟，超过 180 秒过期；
- 新浪场外基金：300 秒内正常，300 至 900 秒延迟，超过 900 秒过期；
- 未知来源：60 秒内正常，60 至 300 秒延迟，超过 300 秒过期。

#### G. 表格

行情表固定为 18 列且严格只读，不允许新增行、删除行或编辑。空值使用“--”，不得使用假 0 兜底。普通窗口使用 DataGrid 内部横向滚动，不得增加页面级横向滚动。

#### H. 数据源状态

固定显示当前状态、最近成功、最近失败、连续失败次数、冷却至、最近错误和更新时间。不得隐藏 `LastError`、`FailureCount` 和 `CooldownUntil`。

#### I. 自动刷新

窗口固定使用 2 秒本地只读 Timer。不得改为网络刷新、增加手动刷新按钮、写入数据库、写入日志或触发业务计算。

#### J. 深色主题

锁定原生深色标题栏、深色窗口边框、专属深色横向及纵向滚动条，以及 Window 和根容器背景 `#050B14`。不得修改全局 ScrollBar 样式。

#### K. 白闪 P0

行情监控窗口固定采用项目历史稳定深色首帧方案：
...
- 禁止 `Thread.Sleep`；
- 禁止固定 `Task.Delay`；
- 禁止 `WindowInteractionEffects.ApplySmoothOpen`；
- 禁止 `WindowStyle=None`；
- 禁止 `AllowsTransparency=True`；
- 禁止 `WindowChrome`；
- `SourceInitialized` 设置 DWM 和 HWND 深色背景；
- 首次本地快照在 `Show` 前完成。
...
不得影响主界面、TradeLog、设置中心、风险中心、回撤图、图表、账户回放、策略计算、委托草案、行情限频和熔断及正式发布规范。
...
### TASK-CAPITAL-POSITION-020：资金仓位中心锁定

锁定基线：

- 正式版本：v8.8.0
- 功能 commit：`ec9cd6d135e956a632ff34a6f27beb198c23cc48`
- 自动化测试：1457/1457
- Release build：0 warning / 0 error
- 功能验收：通过
- 静态 UI 验收：通过
- 白闪状态：仍存在，用户明确接受并暂缓修复

#### A. 入口和窗口

左侧原“资金仓位”入口固定接入资金仓位中心，不得新增第二入口、改名、改变顺序或图标，也不得映射到 `ManualEntryScope`。

资金仓位中心固定为独立单实例窗口；Owner 在首次 `Show` 前设置；首次点击只 `Show`，重复点击只 `Activate`，不使用 `ShowDialog`；关闭后停止 Timer、解除事件并清理 MainWindow 中的窗口引用。窗口固定使用 2 秒本地只读 Timer，不得增加手动刷新按钮。

#### B. 数据边界

统一读取入口固定为 `LocalDataRepository.ReadCapitalPositionReadModel()`：使用同一个 SQLite 连接，先设置 `PRAGMA query_only = ON`，并在同一个只读事务中读取最新 `account_replay_state`、当前 `position_replay_state`、`otc_position_replay_state`、`strategy_config`、`otc_channel`、`market_quote_cache` 和最新有效 `strategy_decision_state`。

该窗口不得读取 TradeLog，不得写数据库或 `runtime_log`，不得创建表或索引，不得修改 schema、缓存或账户回放结果，不得联网，也不得调用 `AccountReplayService`、`StrategyDecisionService` 或委托服务、生成委托草案。

#### C. 顶部 12 项

顶部固定展示以下 12 项，字段、数据来源和显示口径不得另行重算：

1. 总资产：`AccountReplayState.TotalAssets`；
2. 可用现金：`AccountReplayState.CashBalance`；
3. 累计本金：`AccountReplayState.Principal`；
4. 持仓总市值：`AccountReplayState.KnownMarketValue`；
5. 场内 ETF 市值：仅汇总 `position_replay_state` 中 `Source=场内ETF` 的已知市值；
6. 场外基金市值：仅汇总 `otc_position_replay_state` 的已知市值；
7. 持仓浮动盈亏：`AccountReplayState.TotalUnrealizedPnl`；
8. 已实现盈亏：`AccountReplayState.TotalRealizedPnl`；
9. 资金使用率：直接读取 `AccountReplayState.PositionRatio`，不得重算；
10. 狙击资金池：读取最新有效决策的 `RealSniperPool`，无有效值显示“--”，不得回退现金余额；
11. 底仓完成度：读取最新有效决策的 `BaseCompletionRate`，文字显示真实值，图形最高绘制 100%，不得使用 `BasePositionRatio` 替代；
12. 回放/估值状态：使用持久化账户回放状态和完整性标记。

估值不完整时，总资产保留持久化已知值并明确显示估值不完整；所有派生占比显示“--”；不得用成本价、昨收或成交价补估值。

#### D. OTC 防重复

ETF 市值只统计 `position_replay_state` 中 `Source=场内ETF` 的记录；OTC 市值只统计 `otc_position_replay_state`。不得再次累计 `position_replay_state` 中的 OTC 复制行，策略占比不得双算 OTC，窗口不得重算并覆盖 `KnownMarketValue`。同策略多个 OTC、同 ETF 多策略均保持独立展示。

#### E. 仓位结构

仓位结构固定展示现金、场内 ETF、场外基金三项占比；轨道颜色固定为 `#10283A`，现金、ETF、OTC 填充颜色分别为 `#4F8FC9`、`#27A8C4`、`#2A9D8F`，不得增加动画。

策略占比区固定显示策略代码、名称、ETF 市值、OTC 市值、合计市值和总资产占比；不得实现底仓/机动仓拆分。

#### F. ETF 与 OTC 表

ETF 表固定为 14 列：策略代码、ETF 名称、实际代码、持仓数量、综合成本、最新价格、持仓市值、浮动盈亏、盈亏比例、资产占比、行情来源、行情时间、本地接收时间、缓存状态。ETF 名称列宽固定为 240。

OTC 表固定为 15 列：策略代码、基金代码、基金名称、持有份额、持仓成本、单位成本、最新净值、持仓市值、浮动盈亏、盈亏比例、资产占比、通道优先级、净值来源、净值时间、缓存状态。基金名称列宽固定为 270，长名称省略显示并保留完整 ToolTip。

两张表严格只读，不得新增、删除或编辑；只允许 DataGrid 内部横向滚动，不得增加页面级横向滚动；不得增加搜索、筛选、导入、导出、保存、网络刷新、自动调仓或委托执行。

#### G. 深色主题

窗口固定使用原生标题栏、深色 DWM 标题栏和边框，Window 与根容器背景均为 `#050B14`，并使用窗口专属深色滚动条。不得启用 `WindowChrome`、`AllowsTransparency=True`、`Opacity=0`、Reveal、启动遮罩、动画或延迟打开方案。

绝对金额固定使用中性浅色；只有浮动盈亏和已实现盈亏使用正红负绿；正常完整状态使用绿色，估值不完整使用橙色，错误使用红橙色。

#### H. 已知白闪遗留

资金仓位中心在部分实机环境中打开时仍出现白闪。用户在V8.8.0验收阶段明确决定暂缓修复，并授权该问题不阻塞V8.8.0发布。后续重新处理时必须单独解锁本模块。后续新增窗口不得以该豁免作为允许白闪的依据。

不得将该状态表述为已解决、动态验收完成、P0 解除或打开无闪烁。V8.7.0 行情监控中心已锁定的无白闪要求保持不变。

#### I. 跨模块保护

资金仓位中心及其已知白闪豁免不得影响行情监控、TradeLog、设置中心、风险中心、ManualDataEntryWindow、账户回放、持仓回放、策略、委托、行情刷新、缓存、数据库结构和正式发布规范。

### TASK-INDICATOR-DRAWDOWN-021：指标回撤中心锁定

锁定基线：

- 正式版本：v8.9.0
- 功能 commit：`d85f2fdd9e3e64f1bbe8371c70463df9c22ca16c`
- 自动化测试：1563/1563
- Release build：0 warning / 0 error
- 功能验收：通过
- 静态 UI 验收：通过
...
左侧原“指标回撤”入口固定接入指标回撤中心，不得新增第二入口、改名、改变顺序或图标，也不得映射到 `ManualEntryScope`。

指标回撤中心固定为独立单实例窗口；Owner 在首次 `Show` 前设置；首次点击只 `Show`，重复点击复用同一实例并 `Activate`，不使用 `ShowDialog`；关闭后停止 Timer、取消后台任务、解除事件并清理 MainWindow 中的窗口引用。

窗口固定每 2 秒读取本地活动标的、相关 quote 和 source status，每 15 个 Tick（30 秒）检查一次历史元数据签名。不得增加手动刷新按钮。
...
ETF 代码必须标准化并去重；同一 ETF 对应多个启用策略时只显示一行并合并展示策略代码。不得纳入 OTC、账户回撤、策略净值回撤、汇率、未启用策略 ETF 或其它无关缓存标的。

#### C. 计算口径

历史指标统一使用真实前复权日线 `Close`，不得使用最高价、最低价、分时点、昨收或模拟数据替代。

当前回撤固定为：

`latestQuote / max(历史最高收盘价, latestQuote) - 1`

quote 高于历史高点显示 `0.00%（创新高）`，等于历史高点显示 `0.00%`；无有效 quote 时最新价和当前回撤显示 `--`。quote 只影响最新价和当前回撤，不写入历史缓存，也不参与历史最大回撤、20/60/120/252 日回撤或 YTD 回撤。

历史最大回撤固定使用 running peak：

`runningPeak[t] = Max(Close[0..t])`

`drawdown[t] = Close[t] / runningPeak[t] - 1`

`maxDrawdown = Min(drawdown[t])`

相同历史高点保留第一次日期，相同最大回撤保留第一次谷底日期；峰值日期和谷底日期必须保持正确。单调上涨最大回撤为 0 且区间为 `--`；少于 2 个有效点时最大回撤为 `--`。

20/60/120/252 日分别使用最后 20/60/120/252 个有效历史交易点。数据不足时只使用现有真实点降级计算并明确显示“数据不足”，不得补点或插值。YTD 使用最后历史点所属年份，从该年第一条有效历史点开始计算。

#### D. 数据读取边界

专用只读入口固定为：

- `ReadIndicatorDrawdownReadModel`；
- `ReadIndicatorDrawdownRealtimeState`；
- `ReadIndicatorDrawdownHistoryCandidates`。

读取必须使用 SQLite `mode=ro`、`PRAGMA query_only=ON`、单连接和单只读事务，并只按目标标的批量读取 `strategy_config`、`market_quote_cache`、`market_source_status` 和 `market_history_cache`。

不得读取 TradeLog 或账户快照拼接回撤；不得写数据库、写 `runtime_log`、修改缓存或触发网络请求。普通 2 秒 Tick 不得读取历史 payload、解析全历史或重算全部历史指标；只有历史签名或活动标的集合变化时才读取并重算变化标的。

#### E. 历史来源

ETF 历史首选腾讯前复权日 K（`TENCENT_DAILY_QFQ`），指数历史首选东方财富历史 K 线（`EASTMONEY_HISTORY`）。

同一来源按更新时间和 ID 选择候选；最新 payload 损坏时允许回退到较旧的完整真实 payload，首选来源不可用时允许整体回退到另一已知真实来源。不得跨 payload 拼接，也不得跨来源拼接。

完整历史签名必须包含 source、market type、symbol、row ID、updated time、payload length 和 payload SHA-256。

#### F. 状态与摘要

数据状态优先级固定为：数据损坏、无历史、数据源异常、熔断冷却、限频、历史滞后、数据不足、实时行情缺失、正常。历史截止日与参考日期相差超过 7 个自然日时标记“历史滞后”。

顶部固定 7 张摘要卡：监控标的、正常、数据不足、历史滞后、异常/缺失、当前最深回撤、历史最深回撤。

状态摘要必须满足：

`监控标的 = 正常 + 数据不足 + 历史滞后 + 异常/缺失`

“异常/缺失”按最终互斥状态统计数据损坏、无历史、数据源异常、熔断冷却、限频和实时行情缺失。同一行不得重复计数，也不得把限频或异常标的伪装为正常。

#### G. 表格与详情

主表固定为 18 列：分类、名称、代码、关联策略、最新价、历史最高收盘、高点日期、当前回撤、20 日回撤、60 日回撤、120 日回撤、252 日回撤、年初至今回撤、历史最大回撤、最大回撤区间、历史数据、数据来源、数据状态。

主表严格只读，只允许 DataGrid 内部横向和纵向滚动，不得增加页面级横向滚动。2 秒刷新必须保存并恢复用户排序，不得增加假数据、占位行或完整图表。

底部详情固定三行：

1. 名称、代码、分类、关联策略；
2. 历史来源、实时来源、历史区间、数据点数、历史最高收盘及日期；
3. 当前回撤、历史最大回撤、最大回撤区间、状态和状态说明。

方法说明必须独立显示。长文本截断时必须提供完整 Tooltip，不得缩成单行或新增滚动条。

#### H. 跨模块保护

不得影响主界面旧回撤图、`IndexDrawdownChartSeriesBuilder`、`SecurityChartWindow`、`KLineAggregator`、`MarketDataRefreshService`、TradeLog、账户回放、策略、委托、行情监控、资金仓位、数据库结构或正式发布规范。

不得修改行情 parser、router、scheduler、全局限频或熔断；不得新增主界面手动刷新按钮。

#### I. 已知白闪遗留

指标回撤中心在部分实机环境中打开时仍出现白闪。用户在 V8.9.0 验收阶段明确决定暂缓修复，并授权该问题不阻塞 V8.9.0 发布。后续重新处理时必须单独解锁本模块。后续新增窗口不得以该豁免作为允许白闪的依据。

不得将该状态表述为白闪已修复、动态验收通过、P0 已从技术上解决或打开过程完全无闪烁。本次发布不得继续试验白闪修复代码。

### TASK-T1T6-CHART-CENTER-022：T1-T6看图中心锁定

锁定基线：

- 正式版本：`v8.10.0`
- 功能 commit：`2a742e203f58726dd8666cbd7ce9503b7047b985`
- 自动化测试：`1641/1641`
- Release build：`0 warning / 0 error`
- 功能验收：通过
- 静态 UI 验收：通过
...
1. 左侧原第 6 项“T1-T6看图”的名称、图标 `◱` 和顺序保持不变，不得新增第二入口，不得映射到 `ManualEntryScope`。
2. 中心固定使用独立单实例 `T1T6ChartCenterWindow`。
3. 首次打开必须先读取本地只读数据、构建快照并完成初始绑定，再设置 Owner、注册 Closed，最后仅调用 `Show`。
4. 不得使用 `ShowDialog`，不得首次额外 `Activate` 或 `Focus`，不得先 Show 再 Hide。
5. 重复点击必须复用同一窗口；最小化时先恢复 `Normal`，然后 `Activate`，必要时 `Focus`。
6. 关闭窗口必须停止 2 秒 Timer、解除事件并清理 MainWindow 中的窗口引用；不得关闭已有证券图表窗口。
7. 2 秒 Timer 只能在 Loaded 后启动，只读本地状态，不得新增手动刷新按钮。

#### B. T1-T6 定义

1. 六档名称固定为“狙击一档”至“狙击六档”。
2. 固定指数回撤阈值依次为 `-5%`、`-10%`、`-15%`、`-20%`、`-25%`、`-30%`。
3. 档位权重优先读取 `T1Weight` 至 `T6Weight`，默认权重依次为 `1`、`2`、`4`、`8`、`16`、`32`。
4. 权重为 null、非有限数或小于等于 0 时，必须按档回退对应默认权重。
5. 累计权重为当前档与之前各档权重之和；累计比例为当前累计权重除以六档总权重。
6. 以上定义仅用于展示，不得参与策略重新计算或生产决策。

#### C. 策略身份

1. 只显示 `Enabled=true` 的策略；无持仓的启用策略仍显示，停用策略必须隐藏。
2. 同 ETF 多策略必须按每个 `StrategyCode` 分别显示，不得按 ETF 去重。
3. 持久化决策必须严格按原始 `StrategyCode` 精确匹配，禁止按 ETF 代码回退或串用其它策略的决策。
4. 缺少当前 `StrategyCode` 决策时显示“无决策”，不得伪造或重算。
5. quote 可按标准化 ETF 代码共享；图表也按标准化 ETF 代码共享现有证券图表实例。

#### D. 决策事实边界

1. 当前动作和当前档位只能来自最近持久化 `strategy_decision_state`。
2. 不得调用 `StrategyDecisionService` 重新计算策略，不得通过 `TierEngine` 生成生产结果。
3. 档位条件满足只表示持久化当前指数回撤小于等于固定阈值，不代表买入、成交、执行、已投入或已建仓。
4. 当前建议档位只高亮持久化决策字段精确匹配的档位，不得从回撤重新推导。
5. 不得读取或写入 TradeLog，不得读取或展示委托草案，不得展示目标金额、目标数量、目标价格或成交状态。

#### E. Repository 边界

1. 专用方法固定为 `ReadT1T6ChartCenterReadModel()`。
2. SQLite 读取必须使用 `mode=ro`、`PRAGMA query_only=ON`、单连接、单个 deferred 只读事务和批量查询，不得出现 N+1。
3. 只允许读取启用 `strategy_config`、最新 `strategy_decision_state`、相关 `market_quote_cache` 和相关 `market_source_status`。
4. 不得读取 TradeLog、账户回放、账户快照、持仓回放、场外持仓回放、委托草案、历史 K 线、分时缓存或 `runtime_log`。
5. 本模块不得写数据库、写 `runtime_log`、发起网络请求或调用 `MarketDataRefreshService`。
6. 本地读取失败时必须保留上次成功快照，不得每 2 秒弹出 MessageBox，下一 Tick 仅做本地只读重试。

#### F. 页面结构

1. 顶部固定 4 张摘要卡：启用策略、行情正常、无决策、同标的多策略。
2. 左侧固定为启用策略单选列表，右侧固定为当前策略状态和 T1-T6 六张卡。
3. 六张档位卡固定使用 3 列×2 行布局。
4. 页面只能有一个“打开图表”按钮，并必须保留“派生建议、非成交事实”边界说明。
5. 不得嵌入 K 线、账户资金摘要、目标价格、金额、数量、档位成交状态或委托草案。

#### G. 图表复用

1. “打开图表”必须通过 MainWindow 最小回调调用现有 `ChartWindowManager.OpenOrActivate`。
2. 不得修改 `SecurityChartWindow`，不得修改现有图表 key 或窗口管理语义。
3. 同 ETF 多策略必须共享同一证券图表实例，不得创建策略专属图表实例。
4. 不得复制图表逻辑、嵌入 K 线或绘制 T1-T6 水平线；默认周期保持现有图表行为。
5. ETF 代码无效时必须禁用图表按钮并显示明确 ToolTip，不得使用原始无效代码尝试打开图表。

#### H. 跨模块保护

本模块不得影响：

- `StrategyDecisionService`、`TierEngine`、`OrderDraftService`、TradeLog 和 `AccountReplayService`；
- `SecurityChartWindow`、`ChartWindowManager` 既有行为、`ChartDataService` 和 `KLineAggregator`；
- `MarketDataRefreshService`、`MarketDataClient`、`GlobalMarketRequestScheduler` 和 `MarketCircuitBreaker`；
- 行情监控、资金仓位、指标回撤、数据库结构和正式发布规范。

#### I. 白闪说明

用户完成实机界面检查并授权 V8.10.0 正式发布。自动化收口流程未启动完整 EXE，也未独立执行实机白闪检测。后续如发现 T1-T6看图中心存在白闪，应单独解锁本模块处理。V8.8.0和V8.9.0的白闪豁免不得扩展到本模块或后续窗口。

不得将上述说明表述为 Codex 完成了实机白闪验证、动态白闪自动化测试通过或本模块获得了永久白闪豁免。

### TASK-TRADELOG-SAVE-ATOMIC-024：V8.10.1 TradeLog原子保存、双击防重入与故障安全重试

锁定基线：

- 正式版本：`v8.10.1`
- 功能提交：`7d1c13a2b62957b6bb0aa4a6400bcf170b6cd47b`
- 自动化测试：`1675/1675`
- Release build：`0 warning / 0 error`
- 用户人工验收：通过
...
1. 用户真实发生过连续点击两次保存生成两条 TradeLog 记录。
2. 第二条记录后来被完整修改为第二天真实发生的交易。
3. 当前没有生产 TradeLog 数据损坏的证据。
4. 不执行历史 TradeLog 自动查重、合并、删除、恢复或修复。
5. TradeLog 继续是账户、资金、持仓、成本和盈亏回放的唯一成交事实源。
6. TradeLog 只能由用户在交易日志手动录入保存路径明确写入。
7. TradeLog 核心保存事务必须使用一个 SQLite 连接。
8. TradeLog 核心保存事务必须使用一个 SQLite 事务。
9. 核心事务固定覆盖 `trade_log`、`account_replay_state`、`account_replay_snapshot`、`position_replay_state` 和 `otc_position_replay_state`。
10. 生产保存路径只允许在五张核心表全部成功后执行一次 Commit。
11. Commit 前任一步失败，五张核心表必须整体 Rollback。
12. 禁止产生新 TradeLog 事实与旧派生状态，或旧 TradeLog 事实与新派生状态的混合状态。
13. 权威账户回放必须在新增 TradeLog 获得最终数据库 ID 后执行。
14. 账户回放原有排序固定保持 `Time -> Id -> RowIndex`。
15. 本任务不得修改 `AccountReplayService` 的公式、动作含义、金额、现金、本金、持仓或排序规则。
16. 回放状态为“正常”时允许 Commit。
17. 回放状态为“估值不完整”时允许 Commit，并保留不完整估值状态。
18. 回放状态为“财务异常”时必须整体 Rollback。
19. 财务异常不得持久化错误账户状态或追加错误账户快照。
20. 财务异常不得持久化删除原有 ETF 或 OTC 持仓。
21. Commit 后的 TradeLog ID 映射必须使用不可变保存快照中的 `SnapshotIndex`。
22. 新数据库 ID 只能在 Commit 成功后回填 UI 绑定对象。
23. 禁止按交易内容匹配 ID，禁止内容哈希去重或自动合并相似交易。
24. 用户明确录入的合法相同交易必须允许分别存在并获得不同数据库 ID。
25. UPDATE 未命中预期 ID 时必须整体 Rollback，不得降级为 INSERT。
26. 显式删除必须在同一核心事务中执行；删除未命中预期 ID 时必须整体 Rollback。
27. TradeLog 保存操作必须具有早于第一个异步边界的代码级防重入保护。
28. 保存期间保存按钮必须禁用，但按钮禁用不得作为唯一防重入防线。
29. Rollback 后必须恢复未提交 UI 状态、清除临时 ID 污染并允许安全重试。
30. Commit 成功但 UI 同步失败时，同一窗口不得再次提交 TradeLog 事实。
31. Commit 后 UI 同步失败提示必须明确核心数据已经保存，并要求关闭后重新打开窗口。
32. `DataSaved` 只能在核心 Commit 和 UI 同步都成功后触发。
33. 策略决策与委托草案不得进入 TradeLog 核心保存事务。
34. 本任务不得修改数据库 schema、表、字段、索引或唯一约束。
35. 本任务不得修改 TradeLog 字段、UI 列顺序、列布局持久化或视觉样式。
36. 自动化测试基线固定为 `1675/1675`，失败 0、跳过 0。
37. 用户已明确确认本任务人工验收通过。
38. 用户自行在正常 Windows 账户和生产数据库环境完成实机验收；自动化开发及测试未访问生产数据库。
39. 用户明确确认验收后数据无问题，并授权正式提交、推送、锁定、标签和发布收口。
40. 后续如需修改本节任一行为，必须单独解锁 `TASK-TRADELOG-SAVE-ATOMIC-024`。

### TASK-CLEAN-CHECKOUT-TEST-PORTABILITY-032：V8.10.2 干净检出测试换行兼容性

锁定基线：

- 版本：`V8.10.2`
- 自动化测试：`1682/1682`，失败 0、跳过 0
- 生产实现基线：保持 `TASK-TRADELOG-SAVE-ATOMIC-024` 已验收实现不变

锁定行为：

1. 本任务只修改测试辅助逻辑、测试断言、版本元数据和说明文档，不修改任何产品实现。
2. `MarketMonitorWindowTests.SourceGrid_HasReadableHeightTypographyAndFullErrorToolTip` 必须继续验证行情源表格的可读高度、排版、列结构和完整错误 ToolTip 绑定。
3. `SystemSettingsCenterTests.WindowConstructor_KeepsRequiredBuildLoadScopeAndHealthLifecycleOrder` 必须继续验证 Build、Load、Scope 和运行健康生命周期初始化的存在性与顺序。
4. 源码结构测试不得依赖 LF 或 CRLF；LF、CRLF、混合 LF/CRLF 和 CR-only 输入均必须通过相同语义校验。
5. 删除目标结构、删除关键属性或调换必需调用顺序时，相关测试仍必须失败，禁止通过删减断言换取通过。
6. 本任务不得新增或修改 `.gitattributes`，不得修改仓库、系统或用户级 Git 换行配置。
7. 已推送的 `v8.10.1` annotated tag 保持不动，不得删除、移动、覆盖或重新创建。
8. `v8.10.1` 已完成代码提交、锁定提交和标签，但因最终 clean checkout 换行敏感测试失败而未正式发布。
9. `v8.10.2` 只承接测试可移植性修复及正式发布，不引入新的业务功能。
10. 自动化测试基线固定为 `1682/1682`，失败 0、跳过 0。
11. 后续如需修改本节任一行为，必须单独解锁 `TASK-CLEAN-CHECKOUT-TEST-PORTABILITY-032`。

### TASK-RELEASE-STATIC-VALIDATION-029：V8.10.3 正式发布脚本安全静态验证模式

锁定基线：

- 正式版本：`v8.10.3`
- 功能提交：`c740bf39b356ba1612e298c5cb95ffbfdfd66335`
- 自动化测试：`1720/1720`，失败 0、跳过 0
- 发布专项测试：`43/43`
- Release build：`0 warning / 0 error`
- 用户代码与测试报告验收：通过
...
1. 正式发布脚本固定为 `scripts/Publish-CrossEtfRelease.ps1`。
2. 脚本参数 `ValidationMode` 只允许 `Launch` 或 `Static`，默认值固定为 `Launch`。
3. `Static` 必须显式指定；禁止通过环境变量、CI 标志、机器名或用户账号自动切换模式。
4. 禁止在 `Launch` 失败后自动降级为 `Static`。
5. `Launch` 必须保留正式 EXE 启动、15 秒启动等待、8 秒运行等待、主窗口检查、`CloseMainWindow`、15 秒退出等待及可选快捷方式启动验证。
6. `Static` 禁止启动 `跨境ETF.exe`，禁止启动快捷方式，禁止调用 `CloseMainWindow`、Kill 或 `Stop-Process`。
7. `Static` 禁止访问生产数据库、请求真实行情或生成应用运行副作用。
8. `Launch` 和 `Static` 必须共用完全相同的 publish 参数与静态校验逻辑。
9. 正式发布来源必须同时满足 detached HEAD、exact tag、annotated tag、clean worktree、空暂存区、HEAD 等于完整 `ExpectedCommit`。
10. origin 必须存在同名 annotated tag，远端 tag object 和 peeled commit 必须与本地及 `ExpectedCommit` 一致。
11. 普通 main 工作区不得直接执行正式发布。
12. publish 只能写入 `<OutputRoot>\.__staging_v<Version>_<GUID>` 同卷 staging，不得直接写正式版本目录。
13. staging 内必须完成 AppHost 重命名、核心文件、FileVersion、ProductVersion、ExpectedCommit、AssemblyName、中文 EXE、英文 AppHost 残留及污染校验。
14. staging 内必须计算 `跨境ETF.exe` SHA-256 和确定性 manifest hash。
15. 正式 EXE 名称固定为 `跨境ETF.exe`；程序集 `AssemblyName` 固定为 `CrossETF.Terminal.UiShell.Reference`。
16. 正式包递归禁止数据库、WAL/SHM、SQLite 用户文件、测试程序集、TestResults、日志、临时文件、用户配置、用户备份、故障注入配置、PDB 和快捷方式文件。
17. 正式目标目录不存在时，完整验证后的 staging 必须使用同卷目录移动原子提升。
18. 同版本目录内容一致时，必须按 manifest hash 幂等成功，不覆盖或重写已有目录。
19. 同版本目录内容不一致时必须拒绝覆盖，不删除旧目录，并报告或保留 staging 调查路径。
20. 任何历史版本目录均不得被本发布流程修改、删除或覆盖。
21. 快捷方式必须在正式快捷方式同目录创建临时文件，并在安装前静态验证 `TargetPath`、`WorkingDirectory`、`IconLocation`、`Description` 及目标 EXE 存在性。
22. 已存在快捷方式必须优先使用同目录 `File.Replace` 原子替换；不存在时使用同目录 `File.Move`。
23. 快捷方式安装后必须再次静态复核；验证失败必须原子恢复原快捷方式并复核原 hash 或属性。
24. 快捷方式恢复失败必须保留备份证据并报告路径，不得留下指向错误目标的正式快捷方式。
25. `Static` 可以在显式指定 `CreateDesktopShortcut` 时静态更新快捷方式，但绝不启动快捷方式。
26. 自动测试只允许使用临时 Git 仓库、本地 bare origin、临时 OutputRoot、临时快捷方式目录和临时发布夹具。
27. 自动测试不得访问真实桌面、写正式 artifacts、启动应用 EXE、访问生产数据库、请求外部网络或持久化修改 Git 配置。
28. 本任务只修改发布工具、发布测试、纯版本元数据和发布文档；应用业务代码和数据库代码保持零修改。
29. 本次 `v8.10.3` 正式发布必须从最终 annotated tag 的 clean detached worktree 原样执行新版脚本。
30. 本次正式发布必须显式使用 `ValidationMode=Static`，不得使用人工拼装的等价静态流程。
31. 本次 Static 发布必须自行完成来源验证、staging publish、AppHost 重命名、静态包校验、正式目录原子提升和快捷方式原子替换。
32. Static 发布摘要必须明确显示应用启动和快捷方式启动均为 skipped。
33. 禁止为了发布便利修改数据库路径、业务启动流程、行情路由或其它锁定模块。
34. 禁止将正式 artifacts、桌面快捷方式、数据库、缓存、日志或测试结果提交到 Git。
35. 自动化测试基线固定为 `1720/1720`，失败 0、跳过 0；发布专项测试固定为 `43/43`。
36. Release build 基线固定为 `0 warning / 0 error`。
37. 用户已完成本任务代码和测试报告验收，并授权独立功能提交、独立锁定提交、标签和 Static 正式发布收口。
38. 本节不得与功能提交合并，锁定提交只能修改 `docs/LOCKED_MODULES.md`。
39. 已存在的 `v8.10.0`、`v8.10.1` 和 `v8.10.2` 标签及正式发布目录不得移动、删除或覆盖。
40. 后续修改上述任一行为，必须单独解锁 `TASK-RELEASE-STATIC-VALIDATION-029`。

### TASK-WINDOW-WHITE-FLASH-030：V8.10.5 五个业务窗口首次打开白闪修复

锁定基线：

- 正式版本：`v8.10.5`
- 功能提交：`1dae05e83c9630fb9691f3436895b7248e2e427b`
- 自动化测试：`1730/1730`，失败 0、跳过 0
- Release build：`0 warning / 0 error`
- 用户实机验收：通过

锁定行为：

1. 本任务覆盖且仅覆盖以下五个业务窗口：T1-T6 看图、资金仓位、指标回撤、溢价决策和行情监控。
2. 已确认白闪发生在窗口首帧或原生 HWND 默认客户区背景擦除阶段，不得以业务加载重构代替该根因处理。
3. 五个窗口的 `Window.Background` 和根容器背景必须保持项目不透明深色 `#050B14`。
4. 首帧背景保护必须在 `SourceInitialized` 阶段接入，不得延迟到 `Loaded` 或 `ContentRendered`。
5. 共享 `WindowWhiteFlashGuard` 只负责原生首帧客户区背景、hook 和原生资源生命周期。
6. 首帧 helper 不得负责业务数据加载、查询、绑定、刷新、策略计算或窗口导航。
7. 禁止使用 `Opacity=0` 或其它透明度隐藏窗口来掩盖白闪。
8. 禁止启用 `AllowsTransparency` 或透明窗口来掩盖白闪。
9. 禁止使用 `Thread.Sleep`、固定等待或阻塞 UI 线程来掩盖白闪。
10. 禁止延迟 `Show`/`ShowDialog`、定时显示、截图遮罩或白色 Loading 页面来掩盖白闪。
11. 本任务不得修改 `WindowChrome`。
12. 本任务不得修改系统标题栏、自绘标题栏或窗口按钮行为。
13. 本任务不得修改窗口 Owner、位置、尺寸、最大化/还原、Show 方式或单实例规则。
14. 本任务不得修改任何业务计算、数据绑定或构造函数业务加载顺序。
15. 本任务不得修改数据库、schema、缓存、用户数据或生产数据库路径。
16. 本任务不得修改行情接口、parser、router、scheduler 或其它网络层行为。
17. 本任务不得修改既有随机 2-4 秒行情刷新口径及任何窗口本地刷新周期。
18. 每个窗口最多挂载一套原生首帧 hook；重复接入必须幂等。
19. 窗口或 `HwndSource` 关闭后必须移除 hook 并释放 GDI 画刷；多次打开关闭不得持续增长原生资源。
20. `GetClientRect`/`FillRect` 必须覆盖当前完整客户区，不得使用固定像素尺寸，不得破坏 DPI、缩放、最大化或还原。
21. 行情监控旧的重复首帧背景逻辑已经合并到统一 helper，禁止重新叠加第二套 hook。
22. 用户已实机验证五个窗口首次打开、多次打开关闭、单实例、标题栏、最大化和还原均正常且无白闪。
23. 自动化测试基线固定为 `1730/1730`，失败 0、跳过 0；Release build 固定为 `0 warning / 0 error`。
24. 后续修改上述任一行为，必须单独解锁 `TASK-WINDOW-WHITE-FLASH-030`。

### TASK-V8.10.10-RELEASE-LOCK-PUBLISH-040：运行数据与主界面稳定性正式锁定
...
- Release build：`0 warning / 0 error`
- 发布准备提交：`5a9db660c548022a4f5c4441846179397e5877c9`
- 本轮续验功能提交：
...
- 最终锁定提交由承载本节的 Git 提交确定，并由 annotated tag `v8.10.10^{}` 唯一确认。

锁定行为：

1. 当日盈亏在首次获得有效行情后连续刷新稳定，不得在有效值和空值之间反复切换。
2. `159941` 及其它场内、场外、场内加场外持仓的持仓盈亏继续使用已验收的券商摊薄口径。
3. 主界面随机刷新不得重建无变化的静态视觉树，不得恢复整体闪烁；局部数值更新保持现有行为。
4. 真实日 K 数据必须持久化，程序重启后必须保留并继续按既有尾部补齐规则工作。
5. 本轮未修改且后续不得借稳定性修复改写 `TradeLog` 事实、现金、本金、总资产口径、策略算法、委托草案算法或数据库结构。
6. 本轮未修改行情数据源和既有刷新频率，不得绕过现有限频及调度边界。
7. 用户明确确认本轮续验结论达到 `A`；不得补写或伪造未提供的采样数据。
8. 正式支持环境为 Windows 10/11 x64；Windows 7 支持已取消。
9. 正式程序使用 .NET 8、`win-x64`、self-contained、非 single-file、非 trimmed 发布。
10. 正式安装程序保持未签名；SmartScreen 显示“未知发布者”属于用户已接受的发布边界，不得创建自签名证书。
11. 安装目录固定为 `%LocalAppData%\Programs\CrossETF`；安装、覆盖安装和卸载均不得删除 `%LocalAppData%\CrossETF.Terminal.UiShell.Reference\cross_etf_terminal.db`。
12. 正式安装包不得包含数据库、WAL/SHM、日志、PDB、测试程序集、用户缓存或用户隐私数据。
13. `v8.10.10` 必须是指向最终锁定提交的 annotated tag；既有 `v8.10.5` 标签不得移动、删除或覆盖。
14. 后续修改上述任一行为，必须单独解锁 `TASK-V8.10.10-RELEASE-LOCK-PUBLISH-040`。
