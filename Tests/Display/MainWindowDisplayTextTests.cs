...
public class MainWindowDisplayTextTests
{
    ...
    public void MainWindow_VersionDisplayUsesAssemblyVersion()
    {
        ...
        Assert.Contains("<Version>8.1.1</Version>", project, StringComparison.Ordinal);
        Assert.Contains("<InformationalVersion>8.1.1</InformationalVersion>", project, StringComparison.Ordinal);
        Assert.Equal("V8.1.1", MainWindow.ResolveDisplayVersion());
        Assert.Equal("版本： V8.1.1", MainWindow.BuildVersionDisplayText());
    }
